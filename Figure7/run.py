...
# 3. Flatten multi-level column names into single-level column names,
# ignoring the 'Unnamed' parts.
new_columns = []
last_main = ""
...
# 6. Create four subplots, arranged vertically, sharing the X-axis, with
# constrained_layout set to False.
fig, axes = plt.subplots(
    4, 1, figsize=(20, 15), sharex=True, constrained_layout=False
)
...
x_new = x.copy()
if len(x_new) > 1:
    # Move the last set of data one unit to the right to create a gap.
    x_new[-1] += 1
...
# 12. Define the conversion types, corresponding column names, and
# conversion names.
...
for idx, (transition, transition_name) in enumerate(
    zip(transitions, transition_names)
):
    ax = axes[idx]
    ...
    if corrected_cases_col not in df.columns or speedup_col not in df.columns:
        print(
            f'列 "{corrected_cases_col}" 或 "{speedup_col}" 未找到。请检查列名。'
        )
        continue

    # 17. Obtain the corresponding conversion for Corrected cases and SpeedUp
    # Over Pytorch, including the "Overall" row.
    ...
    if len(speedup) != len(x_new):
        print(
            f'警告: "{transition} SpeedUp Over Pytorch" 的长度 {len(speedup)} 与 operators 的长度 {len(x_new)} 不匹配。'
        )
        continue
...
# 27. Create a unified legend, positioned at the top center of the entire
# chart, and increase the font size.
fig.legend(
    handles=handles, labels=labels, loc="upper center", fontsize=24, ncol=3
)

# 28. Add shared y-axis titles on the left and right sides, ensuring they
# are positioned outside the chart.
fig.text(
    0.02,
    0.5,
