...
class LoopUnrollOptimizer(c_ast.NodeVisitor):
    """AST Visitor to identify 'For' loops and insert a Pragma node immediately
    preceding them in the Compound block (the function body)."""

    def __init__(self, unroll_factor=None):
        self.unroll_factor = unroll_factor

    def visit_Compound(self, node):
        """Processes a Compound block ({...}) to insert pragmas before
        loops."""
        if node.block_items:
            new_items = []
...
def preprocess_cuda_code(code_str):
    """Cleans CUDA/C code for pycparser: removes comments, directives, and CUDA
    keywords."""
    # 1. Remove comments (Must be done first to prevent issues with directives
    # inside comments)
