...
def loop_recovery(tester, code, source_platform, target_platform):
    try:
        final_code = run_loop_recovery(code, source_platform)
        if not tester.unit_test(final_code)[0]:
...
def stmt_split(tester, code, source_platform, target_platform):
    try:
...
def detensorization(tester, code, source_platform, target_platform):
    try:
...
def loop_fusion(tester, code, source_platform, target_platform):
    try:
...
def loop_reorder(tester, code, source_platform, target_platform):
    try:
...
def loop_split(tester, code, source_platform, target_platform):
    try:
...
def loop_contraction(tester, code, source_platform, target_platform):
    try:
...
def auto_bind(tester, code, source_platform, target_platform):
...
def auto_cache(tester, code, source_platform, target_platform):
    ...
    try:
        ...
        if not unit_test(tester, cache_code):
...
def auto_tensorization(tester, code, source_platform, target_platform):
    try:
        code = run_code_decoration(code)
...
def auto_pipeline(tester, code, source_platform, target_platform):
